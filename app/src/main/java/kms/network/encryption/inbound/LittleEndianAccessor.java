...
public class LittleEndianAccessor {
    ...
    public final String readAsciiString(final int n, final String s) {
        final byte ret[] = new byte[n];
        for (int x = 0; x < n; x++) {
            ret[x] = readByte();
        }
        return new String(ret, Charset.forName(s));
    }
    ...
    public final String readAsciiString2(final int n) {
        try {
            ...
            Logger.getLogger(LittleEndianAccessor.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    }
    ...
    public final String readMapleAsciiString(String s) {
        return readAsciiString(readShort(), s);
    }
